...
describe('EventBuffer', () => {
  ...
  it('does not replay already-committed chunks when a later chunk fails', async () => {
    ...
    // First chunk succeeds, second chunk fails mid-batch.
    const insertSpy = vi
      .spyOn(ch, 'insert')
      .mockResolvedValueOnce(undefined as any)
    ...
    // Committed chunk trimmed; failed chunk retained (no data loss / no replay).
    expect(insertSpy).toHaveBeenCalledTimes(2);
    expect(await eb.getBufferSize()).toBe(2);

    // Retry: only the remaining chunk is inserted — committed events are NOT
    // re-sent, so no duplicates.
    insertSpy.mockReset();
    ...
    const retryValues = insertSpy.mock.calls[0]![0].values as any[];
    const retryNames = retryValues.map((e) => e.name);
    expect(retryNames).toEqual(['event2', 'event3']);
    expect(retryNames).not.toContain('event0');
    expect(retryNames).not.toContain('event1');
  ...
  it('sends a deterministic dedup token so retried inserts are idempotent', async () => {
    ...
    // must match so ClickHouse rejects the duplicate insert.
